...
import { listAnimes, getHome } from '../api'
...
const sorted = computed(() => {
  ...
  else if (sort.value === 'recent') arr.sort((a, b) => (b.release_year || 0) - (a.release_year || 0))
...
onMounted(async () => {
  try {
    // TODO(백엔드): GET /api/v1/animes/ 미구현
    const data = await listAnimes()
    ...
    try {
      ...
      items.value = home.popular_animes || []
    } catch (_) {
      items.value = []
    }
  } finally {
...
<template>
  <div class="wrap">
    ...
    <div class="list-head">
      <div class="lt">애니메이션 <span class="cnt">{{ sorted.length }}작품</span></div>
      <div class="sortbar">
    ...
    <div v-if="usingFallback" class="todo-banner">
      ...
      전체 애니 목록 API(<code>GET /api/v1/animes/</code>)는 아직 백엔드 미구현이라 임시로 인기 애니를 보여주고 있어요.
    </div>

    <div v-if="loading" class="state-msg">불러오는 중…</div>
    <div v-else-if="!sorted.length" class="state-msg"><div class="big">작품이 없습니다</div>시드 데이터를 넣거나 목록 API를 구현해 주세요.</div>
    <div v-else class="grid">
      <WorkCard v-for="a in sorted" :key="a.id" :work="a" kind="anime" variant="grid" />
    </div>
  </div>
</template>
