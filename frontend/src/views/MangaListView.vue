...
async function load(reset = false) {
  ...
  error.value = false
  try {
    const data = await listMangas({ sort: sort.value, page: page.value, page_size: 30 })
...
<template>
  <div class="wrap">
    ...
    <div class="list-head">
      <div class="lt">만화 <span class="cnt">{{ total }}작품</span></div>
      <div class="sortbar">
    ...
    <template v-else>
      <div class="grid">
        <WorkCard v-for="m in items" :key="m.id" :work="m" kind="manga" variant="grid" />
      </div>
